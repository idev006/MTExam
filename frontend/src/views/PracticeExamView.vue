...
<template>
  <PageContainer>
    <PageHeader eyebrow="โหมดฝึกทำข้อสอบ" :title="bank?.title ?? 'ข้อสอบ PDPA'" description="ระบบจะแสดงคะแนนและเฉลยหลังจากกดส่งข้อสอบเท่านั้น">
    ...
    <section v-else-if="isFinalSubmitted && bank" class="space-y-6">
      <div class="card border border-success/30 bg-base-100 shadow-sm"><div class="card-body items-center text-center"><p class="text-sm text-base-content/60">ส่งข้อสอบเรียบร้อยแล้ว</p><p class="text-6xl font-bold text-success">{{ score }}<span class="text-2xl text-base-content/50"> / {{ bank.questions.length }}</span></p><p class="text-base-content/70">เฉลยและเหตุผลประกอบอยู่ด้านล่าง</p></div></div>
    ...
    <section v-else-if="bank" class="card border border-base-300 bg-base-100 shadow-sm"><div class="card-body gap-6">
      ...
      <p v-if="answeredCount !== bank.questions.length" class="text-right text-sm text-warning">ตอบให้ครบทุกข้อก่อนส่งข้อสอบ</p>
    </div></section>
    <ConfirmModal :open="showSubmitModal" title="ยืนยันการส่งข้อสอบ" message="หลังจากส่งแล้ว ระบบจะแสดงคะแนนและเฉลย และไม่สามารถแก้คำตอบรอบนี้ได้" confirm-label="ยืนยันส่งข้อสอบ" cancel-label="กลับไปตรวจคำตอบ" @confirm="confirmSubmit" @cancel="showSubmitModal = false" />
  </PageContainer>
</template>
