...
### AUTH-007 — Separation of exam duties

- `exam_author` creates and revises assessment content but cannot create a new Exam Window.
- `exam_coordinator` creates Exam Windows only from Published Exam Creations whose quota buckets
  intersect the coordinator's assigned organization scope.
- A coordinator manages only Windows they created; `super_admin` is the audited override.
- An author may finish only a legacy Window that the same author created before this separation was
  introduced. This transition permission does not allow creation of another Window.
- Frontend capability visibility is informational; every API mutation rechecks role, creator and scope.

## Roles

Super Admin, Division Admin, Bureau Admin, Station Admin, Exam Author, Exam Coordinator, Examinee และ Viewer ตาม Master Blueprint
