...
class ExamPaper(Base):
    ...
    family_id: Mapped[UUID | None] = mapped_column(Uuid, index=True)
    revision_number: Mapped[int] = mapped_column(Integer, default=1)
    based_on_paper_id: Mapped[UUID | None] = mapped_column(Uuid, index=True)
    updated_at: Mapped[datetime | None] = mapped_column(DateTime)
    updated_by: Mapped[UUID | None] = mapped_column(Uuid, ForeignKey("persons.id"))
