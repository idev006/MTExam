...
def start_or_resume(
    ...
    account: Annotated[
        UserAccount, Depends(require_roles(UserRole.EXAMINEE, UserRole.SUPER_ADMIN))
    ],
) -> SessionResponse:
    ...
    existing = db.scalar(
        select(ExamSession).where(
            ExamSession.exam_window_id == window.id, ExamSession.person_id == account.person_id
        )
    )
    ...
    session = ExamSession(
        ...
        examinee_snapshot_text=json.dumps(
            {"person_id": str(account.person_id), "org_unit_id": str(actual_org_id)}
        ),
        org_unit_id=actual_org_id,
        eligibility_org_unit_id=quota_org_id,
        started_at=now,
        ends_at=ends,
    ...
    record_audit(
        ...
        event_type="exam_session.start",
        subject_type="exam_session",
        subject_id=session.id,
        metadata={"window_id": str(window.id)},
    )
    db.commit()
...
def save_answer(
    ...
    if (
        question is None
        or choice is None
        or choice.question_id != _question_id_from_version(question.question_version_id, db)
    ):
    ...
    answer = db.scalar(
        select(ExamAnswer).where(
            ...
            ExamAnswer.exam_variant_question_id == question.id,
        )
    )
    if answer is None:
        answer = ExamAnswer(
            exam_session_id=session.id,
            exam_variant_question_id=question.id,
    ...
    record_audit(
        db,
        actor_person_id=account.person_id,
        event_type="exam_session.answer",
        subject_type="exam_session",
        subject_id=session.id,
        metadata={"variant_question_id": str(question.id)},
    )
    db.commit()
    return _response(session, db)
...
def _owned_session(session_id: UUID, account: UserAccount, db: Session) -> ExamSession:
    ...
    if session is None or (
        session.person_id != account.person_id and account.role != UserRole.SUPER_ADMIN
    ):
...
def _ensure_variant(paper: ExamPaper, db: Session, person_id: UUID | None = None) -> ExamVariant:
    variants = list(
        db.scalars(
            ...
            .order_by(ExamVariant.variant_label)
        )
    )
    if not variants:
        variants = []
        ...
        for variant_index in range(paper.variant_count):
            ...
            variant = ExamVariant(
                exam_paper_id=paper.id, variant_label=label, generation_seed_reference=seed
            )
            db.add(variant)
            db.flush()
...
def _response(session: ExamSession, db: Session) -> SessionResponse:
    ...
    answers = {
        str(row.exam_variant_question_id): str(row.selected_choice_id)
    ...
    return SessionResponse(
        id=session.id,
        exam_window_id=session.exam_window_id,
